...
function custom_breadcrumbs() {
  ...
  $breadcrums_class   = 'list-inline list-decor list-menu list-angle-right fs-xs mb-0';
  $home_title         = 'Главная';
  ...
  if ( !is_front_page() ) {
      ...
      // Home page
      echo '<li class="item-home liclean"><a class="bread-link bread-home" href="' . get_home_url() . '" title="' . $home_title . '">' . $home_title . '</a></li>';
      ...
      if ( is_archive() && !is_tax() && !is_category() && !is_tag() ) {
          ...
          if(!empty($last_category)) {
              echo $cat_display;
              echo '<li class="item-current item-' . $post->ID . '"><span class="bread-current bread-' . $post->ID . '" title="' . get_the_title() . '">' .  mb_strimwidth(get_the_title(), 0, 50, '...') . '</span></li>';
